...
export default function SchoolPage() {
  ...
  const addSubtask = (assignment: HomeworkAssignment) => {
    ...
    updateAssignment(
      assignment.id,
      { subtasks: [...assignment.subtasks, { id: makeId('subtask'), title, completed: false }] },
      'Subtask added.',
    )
    ...
    updateAssignment(
      assignment.id,
      {
        subtasks: assignment.subtasks.map((subtask) =>
          subtask.id === subtaskId ? { ...subtask, ...change } : subtask,
        ),
      },
      message,
    )
  ...
  return (
    <>
      <PageHeader
        title="School"
        description="Turn deadlines into a realistic plan around the commitments already on your calendar."
        action={
          <button className="button button--primary" type="button" onClick={() => setAssignmentModal('add')}>
            <Plus aria-hidden="true" /> Add homework
          </button>
        }
      />
      <div className="subnav-row">
        ...
        />
        {view === 'planner' ? (
          <button className="button button--accent" type="button" onClick={generate}>
            <Sparkles aria-hidden="true" /> Generate plan
          </button>
        ) : null}
      </div>
      ...
      {view === 'homework' ? (
        <div className="school-layout">
          <div className="assignment-list">
                ...
                <Card key={assignment.id} className="assignment-card" as="article">
                  <div className="assignment-card__top">
                    ...
                    <div className="assignment-card__badges">
                      <StatusBadge tone={assignment.status === 'complete' ? 'success' : 'neutral'}>
                        {statusLabel[assignment.status]}
                      </StatusBadge>
                      <StatusBadge
                        tone={
                          relativeDueLabel(assignment.dueDate) === 'Overdue'
                            ? 'danger'
                  ...
                  <div className="assignment-card__meta">
                    <span>
                      <Clock3 aria-hidden="true" /> {assignment.estimatedMinutes} min estimated
                    </span>
                    <span>
                      <CalendarPlus aria-hidden="true" /> {formatDate(assignment.dueDate)} at{' '}
                      {formatTime(assignment.dueTime)}
                    </span>
                  </div>
                  {assignment.sourceLabel || sourceUrl ? (
                    <div className="source-line">
                      <span>
                        Source: <strong>{assignment.sourceLabel || 'Imported link'}</strong>
                        {assignment.source === 'imported' ? ' · Imported' : ''}
                      </span>
                      {sourceUrl ? (
                  ...
                  <div className="assignment-progress">
                    <label htmlFor={`progress-${assignment.id}`}>
                      Progress <strong>{assignment.progress}%</strong>
                    </label>
                    <input
                  ...
                  <section className="subtasks" aria-labelledby={`subtasks-${assignment.id}`}>
                    <h3 id={`subtasks-${assignment.id}`}>
                      Subtasks{' '}
                      <span>
                        {assignment.subtasks.filter((item) => item.completed).length}/{assignment.subtasks.length}
                      </span>
                    </h3>
                    <ul>
                      {assignment.subtasks.map((subtask) => (
                        <li key={subtask.id}>
                          <input
                            ...
                            aria-label={`Mark ${subtask.title} ${subtask.completed ? 'not complete' : 'complete'}`}
                            onChange={() =>
                              updateSubtask(
                                assignment,
                                subtask.id,
                          ...
                          <button
                            ...
                            onClick={() =>
                              updateAssignment(
                                assignment.id,
                                { subtasks: assignment.subtasks.filter((item) => item.id !== subtask.id) },
                                'Subtask deleted.',
                              )
                            }
                          >
                    ...
                    <form
                      ...
                      onSubmit={(event) => {
                        ...
                        addSubtask(assignment)
                      }}
                    >
                      <label className="sr-only" htmlFor={`new-subtask-${assignment.id}`}>
                        New subtask for {assignment.title}
                      </label>
                      <input
                        id={`new-subtask-${assignment.id}`}
                        value={subtaskDrafts[assignment.id] ?? ''}
                        placeholder="Add a next step…"
                        onChange={(event) =>
                          setSubtaskDrafts((previous) => ({ ...previous, [assignment.id]: event.target.value }))
                        }
                      />
                  ...
                  <div className="assignment-card__actions">
                    <div>
                      <button
                        ...
                        type="button"
                        onClick={() => setAssignmentModal(assignment)}
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          updateAssignment(
                            ...
                            `${assignment.title} marked complete.`,
                          )
                        }
                      >
                      ...
                      onClick={() => {
                        if (window.confirm(`Delete “${assignment.title}”?`))
                          updateData(
                            (previous) => ({
                              ...previous,
                              assignments: previous.assignments.filter((item) => item.id !== assignment.id),
                              events: previous.events.filter((event) => event.assignmentId !== assignment.id),
                            }),
                            'Assignment deleted.',
                          )
                      }}
            ...
            {!assignments.length ? (
              <EmptyState title="Homework is clear" detail="Add your next assignment when it arrives." />
            ) : null}
          </div>
          <Card className="planner-summary">
            ...
            <h2>Plan around real life</h2>
            <p>
              MyHub plans through each deadline, respects calendar conflicts and your avoid-time ranges, and keeps
              manual study changes.
            </p>
            <dl>
              <div>
                <dt>Available window</dt>
                <dd>
                  {formatTime(data.settings.study.earliestTime)}–{formatTime(data.settings.study.latestTime)}
                </dd>
              </div>
              <div>
                <dt>Default focus</dt>
                <dd>{data.settings.study.defaultBlockMinutes} min</dd>
              </div>
              <div>
                <dt>Avoid ranges</dt>
                <dd>{data.settings.study.avoidTimes.length}</dd>
              </div>
            </dl>
            <button
              ...
              onClick={() => {
                generate()
                setParams({ view: 'planner' })
              }}
            >
              <Sparkles aria-hidden="true" /> Build my study plan
            </button>
          </Card>
        </div>
      ) : (
        <div className="planner-layout">
          <Card className="planner-brief">
            <div>
              ...
              <div>
                ...
                <p>
                  Generated blocks avoid calendar conflicts. Lock, move, or resize a block and MyHub will preserve it
                  next time.
                </p>
              </div>
            </div>
          ...
          <div className="study-block-list">
            {studyBlocks.map((block) => (
              <Card key={block.id} className={block.completed ? 'study-row is-complete' : 'study-row'} as="article">
                <time>
                  <strong>{formatDate(block.date, { weekday: 'short', month: 'short', day: 'numeric' })}</strong>
                  <span>
                ...
                <div>
                  <h2>{block.title}</h2>
                  <p>
                    {block.course} · {block.userAdjusted ? 'Manually adjusted' : 'Generated by MyHub'}
                  </p>
                </div>
                <div className="study-row__actions">
                  <button
                    ...
                    type="button"
                    aria-label={`${block.locked ? 'Unlock' : 'Lock'} ${block.title}`}
                    onClick={() =>
                      updateData(
                        (previous) => ({
                          ...previous,
                          events: previous.events.map((event) =>
                            event.id === block.id ? { ...event, locked: !event.locked } : event,
                          ),
                        }),
                        block.locked ? 'Study block unlocked.' : 'Study block locked.',
                      )
                    }
                  >
                    {block.locked ? <Lock aria-hidden="true" /> : <LockOpen aria-hidden="true" />}
                  </button>
                  <button
                    className="button button--quiet"
                    type="button"
                    onClick={() =>
                      updateData(
                        (previous) => ({
                          ...previous,
                          events: previous.events.map((event) =>
                            event.id === block.id ? { ...event, completed: !event.completed, locked: true } : event,
                          ),
                        }),
                        block.completed ? 'Study block reopened.' : 'Study block completed.',
                      )
                    }
                  >
                    <Check aria-hidden="true" /> {block.completed ? 'Reopen' : 'Complete'}
                  </button>
                  <button
                    className="icon-button icon-button--danger"
                    type="button"
                    aria-label={`Delete study block ${block.title}`}
                    onClick={() =>
                      updateData(
                        (previous) => ({
                          ...previous,
                          events: previous.events.filter((event) => event.id !== block.id),
                        }),
                        'Study block deleted.',
                      )
                    }
            ...
            {!studyBlocks.length ? (
              <EmptyState
                ...
                detail="Generate a plan and MyHub will find open time before each deadline."
                action={
      ...
      <Modal
        ...
        title={assignmentModal === 'add' ? 'Add homework' : 'Edit homework'}
        description="Keep the assignment details and source together so your plan stays trustworthy."
        onClose={() => setAssignmentModal(null)}
      >
        {assignmentModal ? (
          <AssignmentForm
            key={assignmentModal === 'add' ? 'add' : assignmentModal.id}
            assignment={assignmentModal === 'add' ? undefined : assignmentModal}
  ...
  return (
    ...
    >
      <Field label="Assignment title">
        <input name="title" required autoComplete="off" defaultValue={assignment?.title} placeholder="Lab report…" />
      </Field>
      <Field label="Course">
        <input name="course" required autoComplete="off" defaultValue={assignment?.course} placeholder="CE EN 482…" />
      </Field>
      <div className="form-grid__split">
        <Field label="Due date">
          <input
            ...
            type="date"
            required
            defaultValue={assignment?.dueDate ?? toLocalDate(addDays(new Date(), 2))}
          />
        </Field>
        <Field label="Due time">
          <input name="dueTime" type="time" required defaultValue={assignment?.dueTime ?? '23:59'} />
        </Field>
      </div>
      <div className="form-grid__split">
        ...
        <Field label="Estimated minutes">
          <input
            ...
            step="15"
            required
            defaultValue={assignment?.estimatedMinutes ?? 60}
          />
        </Field>
      </div>
      <div className="form-grid__split">
        <Field label="Status">
          <select name="status" defaultValue={assignment?.status ?? 'not-started'}>
      ...
      <Field label="Notes">
        <textarea name="notes" rows={3} defaultValue={assignment?.notes} placeholder="What does done look like?…" />
      </Field>
      <div className="form-grid__split">
        <Field label="Source label" hint="For example, Canvas, syllabus, or instructor email.">
          <input name="sourceLabel" autoComplete="off" defaultValue={assignment?.sourceLabel ?? 'Manual assignment'} />
        </Field>
        <Field label="Source URL" hint="Only http and https links can be opened.">
          <input
            name="sourceUrl"
            type="url"
            inputMode="url"
            defaultValue={assignment?.sourceUrl}
            placeholder="https://…"
          />
        </Field>
      </div>
      {assignment?.source === 'imported' ? (
        <p className="provenance-note">
          Imported record{assignment.importedAt ? ` on ${new Date(assignment.importedAt).toLocaleString()}` : ''}. Your
          progress and subtasks are preserved on re-import.
        </p>
      ) : null}
      <div className="modal__actions">
        ...
        <button className="button button--primary" type="submit">
          {assignment ? 'Save changes' : 'Add homework'}
        </button>
      </div>
    </form>
  )
