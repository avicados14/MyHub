...
const ingredientsFromLines = (value: string): RecipeIngredient[] =>
  value
    .split('\n')
    ...
    .map((line, index) => {
      ...
      return {
        id: `quick-ingredient-${index}-${name}`,
        name,
...
export default function RecipeEditor({ open, recipe, packagedFoods = [], onClose, onSave }: RecipeEditorProps) {
      ...
      ingredients: current.ingredients.map((ingredient) =>
        ingredient.id === id
          ? {
              ...ingredient,
              ...patch,
              ...(patch.name !== undefined ? { canonicalName: patch.name.toLowerCase().trim() } : {}),
            }
          : ingredient,
      ),
    }))
  ...
  const updateNutrition = (key: keyof Nutrition, value: string) => {
    setDraft((current) => ({
      ...current,
      nutritionPerServing: { ...current.nutritionPerServing, [key]: Number(value) },
    }))
  }
  ...
  const uploadImage = async (file: File | undefined) => {
    ...
    if (validation) {
      setImageMessage(validation)
      return
    }
    if (file.size > IMAGE_WARNING_BYTES)
      setImageMessage(
        `${formatFileSize(file.size)} will be stored inside your local MyHub data. The limit is ${formatFileSize(IMAGE_LIMIT_BYTES)}.`,
      )
  ...
  const submit = () => {
    ...
    const ingredients = draft.ingredients
      ...
      .map((ingredient) => ({
        ...
        canonicalName: ingredient.name.trim().toLowerCase(),
      }))
  ...
  return (
    <Modal
      open={open}
      title={recipe ? `Edit ${recipe.name}` : 'Create a recipe'}
      description="Structured fields keep scaling, groceries, nutrition, and review status dependable."
      onClose={onClose}
    >
      <div className="recipe-editor">
        {draft.needsReview ? (
          <section className="review-banner" aria-label="Recipe needs review">
            ...
            <div>
              ...
              <p>Correct the imported fields below. Missing quantities stay blank until you enter them.</p>
            </div>
            <button
            ...
            >
              <CheckCircle2 aria-hidden="true" /> Mark reviewed
            </button>
          </section>
        ) : (
          <div className="review-complete">
            <CheckCircle2 aria-hidden="true" /> Review complete. Saving will preserve this verified state.
          </div>
        )}
        ...
        <section className="editor-section">
          <div className="editor-section__heading">
            <div>
              ...
              <p>Source, identity, timing, and the version you cook now.</p>
            </div>
            <StatusBadge tone={draft.favorite ? 'danger' : 'neutral'}>
              {draft.favorite ? 'Favorite' : 'Standard'}
            </StatusBadge>
          </div>
          <div className="form-grid">
            ...
            <Field label="Notes" hint="Private corrections, substitutions, or cooking reminders.">
              <textarea
                rows={3}
                value={draft.notes ?? ''}
            ...
            <div className="editor-image-row">
              ...
              <div>
                <label className="button button--secondary" htmlFor={imageInputId}>
                  <ImagePlus aria-hidden="true" /> Upload image
                </label>
                <input
                  className="sr-only"
                  id={imageInputId}
                ...
                <small>
                  {imageMessage ||
                    `Images over ${formatFileSize(IMAGE_WARNING_BYTES)} show a storage warning; maximum ${formatFileSize(IMAGE_LIMIT_BYTES)}.`}
                </small>
              </div>
            </div>
            <div className="form-grid__split">
              <Field label="Original yield">
                <input
                  type="number"
                  min="0.25"
                  step="0.25"
            ...
            <div className="form-grid__split">
              ...
              <Field label="Cook minutes">
                <input
                  ...
                  onChange={(event) => setDraft({ ...draft, cookMinutes: Number(event.target.value) })}
                />
              </Field>
            </div>
          </div>
        ...
        <section className="editor-section">
          <div className="editor-section__heading">
            <div>
              <h3>Ingredients</h3>
              <p>Leave source quantities blank when unknown. Overrides apply only at the current yield.</p>
            </div>
            <button
              className="button button--secondary"
              type="button"
              onClick={() =>
                setDraft((current) => ({ ...current, ingredients: [...current.ingredients, newIngredient()] }))
              }
            >
              <Plus aria-hidden="true" /> Ingredient
            </button>
          </div>
          <Field
            label="Ingredients"
            hint="Quick entry: one ingredient per line. Editing this replaces the structured rows below."
          >
            <textarea
              aria-label="Ingredients"
              rows={4}
              value={draft.ingredients.map(ingredientToLine).join('\n')}
              onChange={(event) =>
                setDraft((current) => ({ ...current, ingredients: ingredientsFromLines(event.target.value) }))
              }
              placeholder={'2 cup chickpeas\n1 cup rice\nsalt to taste'}
            />
          </Field>
          <div className="ingredient-editor-list">
            {draft.ingredients.map((ingredient, index) => (
              <article key={ingredient.id} className="ingredient-editor-row">
                ...
                <Field label="Quantity">
                  <input
                    ...
                    onChange={(event) =>
                      updateIngredient(ingredient.id, { quantity: parseNullableNumber(event.target.value) })
                    }
                    placeholder="Unknown"
                  />
                </Field>
                ...
                <Field label="Grocery category">
                  <select
                    ...
                    onChange={(event) =>
                      updateIngredient(ingredient.id, { category: event.target.value as GroceryCategory })
                    }
                  >
                ...
                <div className="override-fields">
                  <Field label={`Override at ${draft.currentYield ?? draft.originalYield} servings`}>
                    <input
                      type="number"
                      min="0"
                      step="any"
                      value={ingredient.scaledOverride?.quantity ?? ''}
                      onChange={(event) =>
                        updateIngredient(ingredient.id, {
                          scaledOverride: {
                            yield: draft.currentYield ?? draft.originalYield,
                            quantity: parseNullableNumber(event.target.value),
                            unit: ingredient.scaledOverride?.unit ?? ingredient.unit,
                          },
                        })
                      }
                      placeholder="Auto"
                  ...
                  <Field label="Override unit">
                    <input
                      value={ingredient.scaledOverride?.unit ?? ingredient.unit}
                      onChange={(event) =>
                        updateIngredient(ingredient.id, {
                          scaledOverride: {
                            yield: draft.currentYield ?? draft.originalYield,
                            quantity: ingredient.scaledOverride?.quantity ?? null,
        ...
        <section className="editor-section">
          <div className="editor-section__heading">
            <div>
              <h3>Method</h3>
              <p>Use one clear action per step.</p>
            </div>
            <button
              className="button button--secondary"
              type="button"
            ...
            >
              <Plus aria-hidden="true" /> Step
            </button>
          </div>
          <Field
            label="Steps"
            hint="Quick entry: one instruction per line. Editing this replaces the structured steps below."
          >
            <textarea
              aria-label="Steps"
              rows={4}
              ...
              onChange={(event) =>
                setDraft((current) => ({
                  ...
                  steps: event.target.value
                    .split('\n')
                    .filter((line) => line.trim())
                    .map((text, index) => ({ id: `quick-step-${index}-${text}`, text })),
                }))
              }
          ...
          <div className="step-editor-list">
            {draft.steps.map((step, index) => (
              <div key={step.id} className="step-editor-row">
                <span>{index + 1}</span>
                <Field label={`Step ${index + 1}`}>
                  <textarea rows={2} value={step.text} onChange={(event) => updateStep(step.id, event.target.value)} />
                </Field>
                <button
                  className="icon-button icon-button--danger"
                  type="button"
                  ...
                  onClick={() =>
                    setDraft((current) => ({ ...current, steps: current.steps.filter((item) => item.id !== step.id) }))
                  }
                >
                  <Trash2 aria-hidden="true" />
                </button>
        ...
        <section className="editor-section">
          ...
          <div className="form-grid__split">
            <Field label="Nutrition source">
              <select
                value={draft.nutritionProvenance.kind}
                onChange={(event) => updateProvenance({ kind: event.target.value as NutritionProvenance['kind'] })}
              >
                <option value="manual">Manual</option>
                <option value="nutrition-label">Nutrition label</option>
                <option value="database">Database</option>
                <option value="recipe-calculation">Recipe calculation</option>
            ...
            <label className="check-control">
              <input
                type="checkbox"
                checked={draft.nutritionProvenance.estimated}
                onChange={(event) => updateProvenance({ estimated: event.target.checked })}
              />
              <span>Mark nutrition as estimated</span>
            </label>
          </div>
          <div className="form-grid__split">
            <Field label="Nutrition source label">
              <input
                value={draft.nutritionProvenance.sourceLabel ?? ''}
                onChange={(event) => updateProvenance({ sourceLabel: event.target.value })}
              />
            </Field>
            <Field label="Nutrition source URL">
              <input
                type="url"
                value={draft.nutritionProvenance.sourceUrl ?? ''}
                onChange={(event) => updateProvenance({ sourceUrl: event.target.value || undefined })}
              />
            </Field>
          </div>
          {draft.needsReview ? (
            <Field label="Review notes">
              <textarea
                ...
                onChange={(event) => setDraft({ ...draft, reviewNotes: event.target.value })}
                placeholder="What still needs correction?"
              />
            </Field>
          ) : null}
        </section>
        ...
        <div className="modal__actions editor-actions">
