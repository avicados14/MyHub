...
export default function FoodLogEditor({ open, data, onClose, onSave, onCreatePackage }: FoodLogEditorProps) {
  ...
  useEffect(() => {
    if (!open) return
    const initial = data.recipes.length
      ? 'recipe'
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : 'Barcode lookup failed. Use custom food or add a packaged food manually.',
      )
    } finally {
      setBusy(false)
    }
  ...
  const scan = async (file: File | undefined) => {
    ...
    try {
      ...
      setNutrition(label.nutrition)
      setProvenance({
        kind: 'nutrition-label',
        capturedAt: new Date().toISOString(),
        estimated: true,
        sourceLabel: 'Browser OCR from nutrition label',
      })
      setConfirmed(false)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'The label could not be read. Enter a custom food manually.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const save = () => {
    ...
    if (source === 'recipe') {
      ...
      if (recipe)
        entry = {
          ...
          name: recipe.name,
          servings,
          nutritionSnapshot: multiplyNutrition(recipe.nutritionPerServing, servings),
          provenanceSnapshot: { ...recipe.nutritionProvenance },
          sourceSnapshot: {
            sourceType: 'recipe',
            sourceId: recipe.id,
            name: recipe.name,
      ...
      if (packaged)
        entry = {
          ...
          name: packaged.name,
          servings,
          nutritionSnapshot: multiplyNutrition(packaged.nutritionPerServing, servings),
          provenanceSnapshot: { ...packaged.nutritionProvenance },
          sourceSnapshot: {
            sourceType: 'packaged',
      ...
      if (leftover)
        entry = {
          ...
          servings: Math.min(servings, leftover.servingsRemaining),
          nutritionSnapshot: multiplyNutrition(
            leftover.sourceSnapshot.nutritionPerServing,
      ...
      entry = {
        ...
        createdAt: timestamp,
        updatedAt: timestamp,
        source: source === 'custom' ? 'manual' : 'imported',
        date,
        name: name.trim(),
        servings,
        nutritionSnapshot: multiplyNutrition(nutrition, servings),
        provenanceSnapshot: { ...provenance, capturedAt: timestamp },
        sourceSnapshot: {
          sourceType: origin,
          name: name.trim(),
          nutritionPerServing: { ...nutrition },
          nutritionProvenance: { ...provenance, capturedAt: timestamp },
          capturedAt: timestamp,
        },
        origin,
      }
    ...
    if (!entry) {
      setError('Choose or name a food before logging it.')
      return
    }
    if ((source === 'barcode' || source === 'label') && !confirmed) {
      setError('Confirm the imported or OCR nutrition values before logging.')
      return
    }
    onSave(entry, source === 'leftover' ? { id: sourceId, servings: entry.servings } : undefined)
  }

  const packageResults = data.packagedFoods.filter((item) =>
    `${item.name} ${item.brand ?? ''} ${item.barcode ?? ''}`.toLowerCase().includes(query.toLowerCase()),
  )
  ...
  return (
    <Modal
      open={open}
      title="Log food"
      description="Logged nutrition is an immutable snapshot. Planned meals do not count until consumption is recorded."
      onClose={onClose}
    >
      <div className="food-log-editor">
        <SegmentedControl
          label="Food log source"
          value={source}
          onChange={(value) => changeSource(value as LogSource)}
          options={[
        ...
        {source === 'packaged' ? (
          <>
            <Field label="Search saved packages">
              <input
                type="search"
                value={query}
            ...
            <button className="button button--secondary" type="button" onClick={onCreatePackage}>
        ...
        {source === 'label' ? (
          <section className="log-source-panel">
            <label className="file-drop" htmlFor={labelId}>
              <ScanText aria-hidden="true" />
              <strong>Choose nutrition label image</strong>
              <small>OCR runs locally; review every field.</small>
            </label>
            <input
              id={labelId}
              className="sr-only"
        ...
        {source === 'leftover' ? (
          <Field label="Available leftovers">
            <select value={sourceId} onChange={(event) => setSourceId(event.target.value)}>
              <option value="">Choose leftovers</option>
              {data.leftovers
                .filter((item) => item.servingsRemaining > 0)
                .map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.sourceSnapshot.name} — {item.servingsRemaining} servings
                  </option>
                ))}
            </select>
          </Field>
        ...
        {showManualNutrition ? (
          <>
            ...
            <div className="provenance-strip">
              <StatusBadge tone={provenance.estimated ? 'attention' : 'neutral'}>
                {provenance.estimated ? 'Imported / estimated' : provenance.kind}
              </StatusBadge>
              <span>{provenance.sourceLabel ?? 'No source recorded'}</span>
            </div>
            {source !== 'custom' ? (
              <label className="confirmation-check">
                <input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />
                <span>
                  <strong>I reviewed the values.</strong>
                  <small>Required before imported nutrition enters the log.</small>
                </span>
              </label>
            ) : null}
          </>
