...
const blankFood = (): PackagedFood => {
  ...
  return {
    ...
    nutritionProvenance: {
      ...
      estimated: false,
      sourceLabel: 'Manual package entry',
    },
    image: '',
    notes: '',
    needsReview: false,
...
export default function PackagedFoodEditor({ open, food, onClose, onSave }: PackagedFoodEditorProps) {
  ...
  const uploadPackage = async (file: File | undefined) => {
    ...
    setMessage(
      `${formatFileSize(file.size)} package image stored locally. Maximum ${formatFileSize(IMAGE_LIMIT_BYTES)}.`,
    )
  }
  ...
  const scanLabel = async (file: File | undefined) => {
    ...
    setBusy(true)
    setError('')
    setMessage('Loading browser OCR for this label only…')
    try {
      const result = await recognizeImageText(file, ({ status, progress }) =>
        setMessage(`${status} · ${Math.round(progress * 100)}%`),
      )
      const label = extractNutritionLabel(result.text)
      setDraft((current) => ({
        ...current,
        servingSize: label.servingQuantity
          ? { quantity: label.servingQuantity, unit: label.servingUnit || current.servingSize.unit }
          : current.servingSize,
        nutritionPerServing: label.nutrition,
        nutritionProvenance: {
          kind: 'nutrition-label',
          capturedAt: new Date().toISOString(),
          estimated: true,
          sourceLabel: 'Browser OCR from uploaded nutrition label',
        },
        notes: [current.notes, ...label.warnings, `OCR confidence ${Math.round(result.confidence)}%.`]
          .filter(Boolean)
          .join(' '),
        needsReview: true,
      }))
      ...
      setError(cause instanceof Error ? cause.message : 'The label could not be read. Enter the values manually.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const save = () => {
    if (!draft.name.trim()) {
      setError('Enter the package name before saving.')
      return
    }
    if (draft.servingSize.quantity <= 0) {
      setError('Serving quantity must be greater than zero.')
      return
    }
    if (draft.needsReview && !confirmed) {
      setError('Confirm that you reviewed imported or OCR values against the package before saving.')
      return
    }
    const timestamp = new Date().toISOString()
    onSave({
  ...
  return (
    <Modal
      open={open}
      title={food ? `Edit ${food.name}` : 'Add packaged food'}
      description="Use manual entry, a read-only Open Food Facts lookup, or local label OCR. All imported values require confirmation."
      onClose={onClose}
    >
      <div className="package-editor">
        <SegmentedControl
          label="Package entry method"
          value={mode}
          onChange={(value) => {
            setMode(value as PackageMode)
            setError('')
          }}
          options={[
        ...
        {mode === 'barcode' ? (
          <section className="import-panel">
            ...
            <p className="source-disclosure">
              Database records can be incomplete or incorrect. Imported values are marked estimated until you confirm
              them.
            </p>
          </section>
        ) : null}
        {mode === 'label' ? (
          <section className="import-panel">
            ...
            <div>
              <h3>Nutrition label OCR</h3>
              <p>Upload a clear label image. Tesseract loads lazily and recognition runs in your browser.</p>
            </div>
        ...
        <section className="editor-section">
          <div className="editor-section__heading">
            <div>
              ...
              <p>Manual fallback is always available.</p>
            </div>
            <StatusBadge tone={draft.nutritionProvenance.estimated ? 'attention' : 'food'}>
              {draft.nutritionProvenance.estimated ? 'Imported / estimated' : 'Manual'}
            </StatusBadge>
          </div>
          <div className="form-grid">
              ...
              <Field label="Servings per container">
                <input
                  ...
                  onChange={(event) =>
                    setDraft({
                      ...draft,
                      servingsPerContainer: event.target.value ? Number(event.target.value) : undefined,
                    })
                  }
                />
              </Field>
            ...
            <div className="form-grid__split">
              <Field label="Serving quantity">
                <input
                  ...
                  min="0.01"
                  step="any"
                  value={draft.servingSize.quantity}
                  onChange={(event) =>
                    setDraft({ ...draft, servingSize: { ...draft.servingSize, quantity: Number(event.target.value) } })
                  }
                />
              </Field>
              <Field label="Serving unit">
                <input
                  value={draft.servingSize.unit}
                  onChange={(event) =>
                    setDraft({ ...draft, servingSize: { ...draft.servingSize, unit: event.target.value } })
                  }
                />
              </Field>
            </div>
            <div className="editor-image-row">
              ...
              <div>
                <label className="button button--secondary" htmlFor={packageImageId}>
                  <ImagePlus aria-hidden="true" /> Upload package image
                </label>
                <input
                  id={packageImageId}
                  className="sr-only"
                  type="file"
                  accept="image/*"
                  onChange={(event) => void uploadPackage(event.target.files?.[0])}
                />
              </div>
            </div>
            <Field label="Notes">
        ...
        <section className="editor-section">
          <div className="editor-section__heading">
            <div>
              ...
              <p>Calories are kcal, sodium is mg, and other nutrients are grams.</p>
            </div>
            <StatusBadge tone={draft.needsReview ? 'attention' : 'success'}>
              {draft.needsReview ? 'Needs confirmation' : 'Confirmed'}
            </StatusBadge>
          </div>
          ...
          <div className="form-grid__split">
            ...
            <label className="check-control">
              ...
              />
              <span>Values are estimated/imported</span>
            </label>
          </div>
          {draft.needsReview ? (
            <label className="confirmation-check">
              ...
              <span>
                ...
                <strong>I compared the imported values with the package label.</strong>
                <small>This confirmation is required before save.</small>
              </span>
            </label>
          ) : null}
        </section>
