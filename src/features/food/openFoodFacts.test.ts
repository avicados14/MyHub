...
describe('Open Food Facts lookup', () => {
  it('uses limited v2 product fields and marks imported values estimated', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ status: 1, product: { code: '12345678', product_name: 'Test Bar', brands: 'Acme', serving_size: '40 g', nutriments: { 'energy-kcal_serving': 180, proteins_serving: 6, carbohydrates_serving: 20, fat_serving: 8, fiber_serving: 3, sodium_serving: 0.2 } } }), { status: 200 }))
