...
export const syncLeftoverForMeal = (data: AppData, meal: MealEntry, timestamp: string): AppData => {
  ...
  if (remaining <= 0) {
    ...
    leftovers = data.leftovers.map((leftover) => leftover.id === current.id
      ? { ...leftover, updatedAt: timestamp, servingsRemaining: remaining, sourceSnapshot: { ...meal.sourceSnapshot } }
      : leftover)
  } else {
...
export const logMealConsumption = (data: AppData, mealId: string, consumedServings: number, timestamp: string): AppData => {
  ...
  const entry: FoodLogEntry | null = safeServings <= 0 ? null : {
...
export const moveOrCopyMeal = (
  ...
  const meal = data.meals.find((item) => item.id === mealId)
  if (!meal) return data
  const withoutTarget = data.meals.filter((item) => !(item.date === target.date && item.slot === target.slot) && (mode === 'copy' || item.id !== mealId))
  const next: MealEntry = mode === 'move'
...
export const consumeLeftover = (data: AppData, leftoverId: string, servings: number, timestamp: string, date: string): AppData => {
  ...
  const entry: FoodLogEntry = {
    ...
    sourceSnapshot: { ...leftover.sourceSnapshot, sourceType: 'leftover', sourceId: leftover.id, capturedAt: timestamp },
    origin: 'leftover',
  }
  return {
    ...data,
    foodLog: [...data.foodLog, entry],
    leftovers: data.leftovers.flatMap((item) => item.id !== leftover.id
      ? [item]
