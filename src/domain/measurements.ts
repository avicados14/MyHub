...
export const MEASUREMENT_UNITS: MeasurementUnit[] = [
  { key: 'tsp', label: 'tsp', dimension: 'volume', system: 'us', toBase: 4.92892159375, aliases: ['tsp', 'teaspoon', 'teaspoons'] },
  { key: 'tbsp', label: 'tbsp', dimension: 'volume', system: 'us', toBase: 14.78676478125, aliases: ['tbsp', 'tablespoon', 'tablespoons'] },
  { key: 'cup', label: 'cup', dimension: 'volume', system: 'us', toBase: 236.5882365, aliases: ['cup', 'cups'] },
  { key: 'fl oz', label: 'fl oz', dimension: 'volume', system: 'us', toBase: 29.5735295625, aliases: ['fl oz', 'floz', 'fluid ounce', 'fluid ounces'] },
  { key: 'mL', label: 'mL', dimension: 'volume', system: 'metric', toBase: 1, aliases: ['ml', 'milliliter', 'milliliters', 'millilitre', 'millilitres'] },
  { key: 'L', label: 'L', dimension: 'volume', system: 'metric', toBase: 1000, aliases: ['l', 'liter', 'liters', 'litre', 'litres'] },
  { key: 'oz', label: 'oz', dimension: 'mass', system: 'us', toBase: 28.349523125, aliases: ['oz', 'ounce', 'ounces'] },
  { key: 'lb', label: 'lb', dimension: 'mass', system: 'us', toBase: 453.59237, aliases: ['lb', 'lbs', 'pound', 'pounds'] },
  { key: 'g', label: 'g', dimension: 'mass', system: 'metric', toBase: 1, aliases: ['g', 'gram', 'grams'] },
  { key: 'kg', label: 'kg', dimension: 'mass', system: 'metric', toBase: 1000, aliases: ['kg', 'kilogram', 'kilograms'] },
  ...
  { key: 'each', label: 'each', dimension: 'count', system: 'neutral', toBase: 1, aliases: ['each', 'item', 'items', 'whole'] },
]
...
const chooseDisplayUnit = (baseValue: number, dimension: 'volume' | 'mass', system: MeasurementSystem): MeasurementUnit => {
  const units = MEASUREMENT_UNITS.filter((unit) => unit.dimension === dimension && unit.system === system && unit.toBase !== undefined)
...
export const convertForSystem = (value: number, unit: string, system: MeasurementSystem): ConvertedMeasurement | null => {
  const source = findMeasurementUnit(unit)
  if (!source) return null
  if (source.dimension === 'temperature') return convertMeasurement(value, source.key, system === 'metric' ? '°C' : '°F')
...
export const formatTemperature = (value: number, unit: string, system: MeasurementSystem): string => {
  ...
  if (!converted || findMeasurementUnit(converted.unit)?.dimension !== 'temperature') return `${Math.round(value)}${unit}`
...
export const splitQuantityAndUnit = (input: string): { quantity: number | null; unit: string; remainder: string } => {
  ...
  const unit = MEASUREMENT_UNITS
    .flatMap((item) => item.aliases.map((alias) => ({ alias, key: item.key })))
