...
const assignment = (
  id: string,
  dueDate: string,
  priority: HomeworkAssignment['priority'],
  minutes = 45,
): HomeworkAssignment => ({
  ...base,
  id,
...
describe('study planner', () => {
  it('ranks earlier deadlines before later high-priority work', () => {
    const ranked = rankAssignments([
      assignment('later-high', '2026-09-24', 'high'),
      assignment('soon-low', '2026-09-23', 'low'),
    ])
  ...
  it('avoids conflicts and reports work that cannot fit', () => {
    ...
    const result = generateStudyPlan(
      [assignment('lab', '2026-09-22', 'high', 90)],
      [busy],
      {
        earliestTime: '16:00',
        latestTime: '18:00',
  ...
  it('plans beyond fourteen days while respecting recurring avoid ranges and complex conflicts', () => {
    ...
    const result = generateStudyPlan(
      ...
      events,
      {
        ...
        breakMinutes: 15,
        avoidTimes: [
          { id: 'monday-lunch', label: 'No Mondays after class', days: [1], startTime: '11:15', endTime: '12:00' },
        ],
      },
      new Date(2026, 8, 22, 8),
    )
  ...
  it('reports invalid ranges and non-blocking overlap warnings for direct edits', () => {
    ...
    expect(
      validateStudyBlock({ id: 'block', date: '2026-09-22', startTime: '10:30', endTime: '10:15' }, [existing]),
    ).toEqual({ valid: false, warning: 'End time must be after start time.' })
    expect(
