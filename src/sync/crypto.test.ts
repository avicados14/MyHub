...
describe('encrypted envelopes', () => {
  it('round trips JSON with PBKDF2-SHA-256 and AES-256-GCM', async () => {
    ...
    const envelope = JSON.parse(encrypted) as { kdf: { iterations: number; hash: string }; cipher: { name: string; keyLength: number } }
    ...
    await expect(decryptJson(encrypted, 'correct horse battery staple')).resolves.toEqual({ private: 'academic and food data' })
  })
  ...
  it('rejects ciphertext tampering', async () => {
    ...
    await expect(decryptJson(JSON.stringify(envelope), 'correct horse battery staple')).rejects.toThrow(/incorrect|changed/)
  })
