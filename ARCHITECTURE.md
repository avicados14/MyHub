...
The Playwright launcher selects an available loopback port for local runs so concurrent worktrees do not collide. CI uses deterministic port `4287`. The workflow invokes the unfiltered `npm run test:e2e` command, which executes all 3 viewport projects before deployment. The final integrated branch passed 96 Vitest tests in 19 files and 123 Playwright tests. A separate 42-action exploratory run exercised every primary route and found no console errors, page exceptions, failed HTTP responses, or page-level overflow. A real Chromium session also connected to the private repository, decrypted 27 recipes and 2 calendar feeds, imported 3,328 corrected events, and verified encrypted write-back.
