...
The Playwright launcher selects an available loopback port for local runs so concurrent worktrees do not collide. CI uses deterministic port `4287`. The workflow invokes the unfiltered `npm run test:e2e` command, which executes all 3 viewport projects before deployment. Companion food-hardening and calendar-hardening results must still be merged and rerun before their audit rows are promoted from implementation to final verification.
