...
IndexedDB is origin-and-browser-profile scoped, so credentials do not appear automatically on a second device. The explicit device-pairing flow is the transfer boundary. An unlocked provider exports repository coordinates, token, and passphrase only to an in-memory setup function. That material is encrypted with a random 16-character pairing code, serialized into a five-minute package, and placed after the hash in a GitHub Pages-compatible URL. The code is displayed separately and is not embedded in the QR. The receiving route captures the encrypted package, removes it from browser history, requires the code, then runs the same private-repository verification and remote-first empty-device synchronization as manual connection. The resulting token is re-encrypted in the receiving browser's credential store; the passphrase returns to memory-only handling.
